...
CREATE USER francisco_ricardo_L6 IDENTIFIED BY 123;
GRANT connect , resource, create view TO francisco_ricardo_L6;
connect francisco_ricardo_L6/123;
SPOOL C:\BD1\francisco_ricardo_L6.txt
...
--Q3
--a)
SELECT cs.c_id, cs.skill_id, s.skill_description,
FROM skill s, consultant_skill cs
WHERE s.skill_id = cs.skill_id and cs.c_id = 100;

--b)
SELECT cs.c_id, cs.skill_id,s.skill_description, cs.certification 
FROM skill s, consultant_skill cs
WHERE s.skill_id = cs.skill_id and cs.c_id = 102 and cs.certification = 'Y';

--c)
SELECT pc.c_id, c.c_last, pc.p_id 
FROM consultant c, project_consultant pc
WHERE c.c_id = pc.c_id and pc.c_id = 1;

--d)
SELECT s.skill_description, c.c_first
FROM skill s, consultant c, consultant_skill cs
WHERE s.skill_id = cs.skill_id and c.c_id = cs.c_id and cs.certification = 'Y';

--e) 
SELECT p.project_name 
FROM project p, consultant c, project_consultant pc
WHERE p.p_id = pc.p_id and pc.c_id = c.c_id and c.c_first = 'Mark' and c.c_last = 'Myer';
--correct last name is MayerS
SELECT p.project_name 
FROM project p, consultant c, project_consultant pc
WHERE p.p_id = pc.p_id and pc.c_id = c.c_id and c.c_first = 'Mark' and c.c_last = 'Myers';

--f)
SELECT p.project_name, c.c_first, c.c_last
FROM project p, consultant c, project_consultant pc
WHERE p.p_id = pc.p_id and p.mgr_id = c.c_id;

--g)
SELECT p.project_name, pp.project_name "PROJECT_PARENT"
FROM project p, project pp
WHERE p.p_id = pp.parent_p_id;
